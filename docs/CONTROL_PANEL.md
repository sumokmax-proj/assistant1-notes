...
유저 개입은 두 레이어로 구분한다:
- **Setup Layer:** 프로젝트 초기 또는 새 환경에서 한 번만 하면 되는 작업
- **Runtime Layer:** 프로젝트가 진행되는 동안 반복적으로 필요한 작업
...
| 상황 | 처리 방식 |
|---|---|
| 승인 요청 후 응답 없음 | **작업 중단** — 해당 작업 및 의존 후속 작업 모두 보류 |
| 다음 세션 시작 | 미결 승인 요청 목록을 세션 첫 메시지에 재보고 |
| 응답 전 Claude 재호출 | "이전 요청 [G번호] 아직 미승인 상태입니다. 승인하시겠습니까?" 재질문 |

> **원칙:** 응답 없음 = 거부와 동일하게 처리. 승인 없이는 절대 진행하지 않는다.
