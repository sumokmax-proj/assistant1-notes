...
- **승인 필수 (🔴 STOP):** 돈 이동, 결제 수단 등록, 보안 자격증명 변경, 신규 유료 서비스 가입, 개인정보 제출, 법적 계약 동의
- **승인 권장 (🟡 CAUTION):** 신규 외부 계정 생성, 커뮤니티 최초 포스팅, 가격 변경, 기존 콘텐츠 삭제
- **자율 실행 (🟢 AUTO):** 코드 작성, 문서 갱신, 툴 설치, API 호출(이미 승인된 채널), git 작업, 통계 확인
- **승인 프로토콜:** 🚦 GO/NO-GO 요청 형식으로 텔레그램 보고 → "OK" 받은 후 실행. 미응답 = 작업 중단, 다음 세션 재질문
- **예산 한도:** $100 (무료 방법 우선)
- **사이클:** 실행 → 검증 → 개선 → 실행
- **피벗 기준:** 4주 내 수익 $0이면 모델 재검토
- **Gumroad UI:** Pause payouts 분홍=일시정지, 회색=정상
- **전체 규칙 상세:** [CONTROL_PANEL.md](./CONTROL_PANEL.md) 참조
...
- [CONTROL_PANEL.md](./CONTROL_PANEL.md) — **유저 개입 목록 / 계정 목록 / 권한 리스크 / Go-No-Go 트리거**
